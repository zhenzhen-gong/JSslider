window.onload = function () {
    function byId(id){
      ...
      var index = 0,
          ...
          menuTitles = menuBox.getElementsByClassName("menu-title")
          menu = byId("menu-content")
          menuContents = menu.getElementsByClassName("content-box")
          pre = byId("pre"),
          next = byId("next")
     ...
     function imgSlider(){
        ...
        for(let i = 0;i<menuTitles.length;i++){  
            menuContents[i].setAttribute("data-index",i)
            menuTitles[i].onmouseover = function(){
                var idx = menuContents[i].getAttribute("data-index")
                for(let j = 0;j<menuContents.length;j++){
                    menuContents[idx].style.display = "none"
                }
                menu.style.display = "block"
                menuContents[idx].style.display = "block"
